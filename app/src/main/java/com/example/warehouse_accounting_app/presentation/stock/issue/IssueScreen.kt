...
fun IssueScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                IssueEvent.Success -> {
                    snackbarHostState.showSnackbar("Расход товара оформлен")
                    onSuccess()
                }
                is IssueEvent.Error -> snackbarHostState.showSnackbar(event.message)
            }
        }
    }

    AppScaffold(
        topBar = { AppTopBar(title = "Расход товара", onBack = onBack) },
        ...
        Column(
            ...
            Card(
                ...
                Column(
                    ...
                    AppButton(
                        text = if (state.isSaving) "Оформление…" else "Оформить расход",
