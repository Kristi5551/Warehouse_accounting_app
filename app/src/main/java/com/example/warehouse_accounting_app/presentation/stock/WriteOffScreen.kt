...
fun WriteOffScreen(
    ...
    val viewModel: WriteOffViewModel = viewModel(factory = viewModelFactory)
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                StockOperationEvent.Success -> { snackbarHostState.showSnackbar("Списание оформлено успешно"); onSuccess() }
    ...
    AppScaffold(
        topBar = { AppTopBar(title = "Списание товаров", onBack = onBack) },
        snackbarHost = { SnackbarHost(snackbarHostState) },
    ) { padding ->
        if (state.isLoading) { LoadingContent(); return@AppScaffold }
        var expanded by remember { mutableStateOf(false) }
        var reasonExpanded by remember { mutableStateOf(false) }
        val reasons = listOf("Брак", "Повреждение", "Потеря", "Истёк срок хранения", "Другое")
        Column(
            ...
            ExposedDropdownMenuBox(expanded = reasonExpanded, onExpandedChange = { reasonExpanded = it }) {
                OutlinedTextField(
                    value = state.reason, onValueChange = viewModel::onReasonChange,
                    label = { Text("Причина списания") }, trailingIcon = { ExposedDropdownMenuDefaults.TrailingIcon(reasonExpanded) },
                    modifier = Modifier.fillMaxWidth().menuAnchor(MenuAnchorType.PrimaryEditable), singleLine = true, shape = MaterialTheme.shapes.medium,
                )
                ExposedDropdownMenu(expanded = reasonExpanded, onDismissRequest = { reasonExpanded = false }) {
                    reasons.forEach { r -> DropdownMenuItem(text = { Text(r) }, onClick = { viewModel.onReasonChange(r); reasonExpanded = false }) }
                }
            }
            ...
            AppButton(text = if (state.isSaving) "Оформление…" else "Оформить списание", onClick = viewModel::onSubmit, modifier = Modifier.fillMaxWidth(), enabled = !state.isSaving)
