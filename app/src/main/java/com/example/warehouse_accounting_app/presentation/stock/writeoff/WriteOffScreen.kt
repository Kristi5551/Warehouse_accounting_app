...
private val writeOffReasons = listOf("Брак", "Повреждение", "Потеря", "Истек срок хранения", "Другое")
...
fun WriteOffScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                WriteOffEvent.Success -> {
                    snackbarHostState.showSnackbar("Списание оформлено")
                    onSuccess()
                }
                is WriteOffEvent.Error -> snackbarHostState.showSnackbar(event.message)
            }
        }
    }

    AppScaffold(
        topBar = { AppTopBar(title = "Списание товара", onBack = onBack) },
                ...
                Column(
                    ...
                    ExposedDropdownMenuBox(expanded = reasonExpanded, onExpandedChange = { reasonExpanded = it }) {
                        ...
                        ExposedDropdownMenu(expanded = reasonExpanded, onDismissRequest = { reasonExpanded = false }) {
                            writeOffReasons.forEach { r ->
                                DropdownMenuItem(
                                    text = { Text(r) },
                                    onClick = { viewModel.onReasonChange(r); reasonExpanded = false },
                                )
                            }
                        }
                    }
                    ...
                    AppButton(
                        text = if (state.isSaving) "Оформление…" else "Оформить списание",
