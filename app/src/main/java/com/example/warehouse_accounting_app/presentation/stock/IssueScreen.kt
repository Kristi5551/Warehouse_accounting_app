...
fun IssueScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                StockOperationEvent.Success -> { snackbarHostState.showSnackbar("Расход оформлен успешно"); onSuccess() }
    ...
    AppScaffold(
        topBar = { AppTopBar(title = "Расход товаров", onBack = onBack) },
        ...
        Column(
            ...
            OutlinedTextField(value = state.reason, onValueChange = viewModel::onReasonChange, label = { Text("Основание расхода") }, modifier = Modifier.fillMaxWidth(), singleLine = true, shape = MaterialTheme.shapes.medium)
            ...
            AppButton(text = if (state.isSaving) "Оформление…" else "Оформить расход", onClick = viewModel::onSubmit, modifier = Modifier.fillMaxWidth(), enabled = !state.isSaving)
